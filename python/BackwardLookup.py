...
cur = conn.cursor()
cur.execute("""select LookupValue,LookupsId from Lookups where LookUpTypeID = 1""") # 1 =ip
rows = cur.fetchall()

#For each use dig command
for row in rows:
    digOutPut = commands.getoutput(r'dig @%s -x %s ANY +noall +answer' %( dnsServer, row[0]))
    ...
    if (len(rx) < 1):
            print "add insert"
            cur.execute("""insert into Records (LookupId,Lookupdate,dnsServer, ip)
...
#    # 3. if there is data do full insert
        ...
        for record in rx:
            ...
            values(%s,'1994-11-29','%s','%s','%s','%s',%s)""" % (row[1],dnsServer,record[2],row[0],record[3],record[1]))
...
#    # 5. if no add it
            if (InLookup == None):
                cur.execute("""insert into Lookups(LookUpTypeID,LookupValue,LookupValueUpate,ErrorID,ErrorUpdate)
                        values(2,'%s','1994-11-29',1,'1994-11-29')"""%(record[3]))
